...
update_root_metadata_with_commit_history() {
  ...
  if ! execute_curl_with_retry "${ROOT_META_URL}" "GET" "${METADATA_FILE}"; then
    echo "Failed to download root metadata, it may not exist yet"
...
${NEW_MAPPING}" "${METADATA_FILE}"
  ...
  if ! execute_curl_with_retry "${ROOT_META_URL}" "PUT" "" "${METADATA_FILE}"; then
    echo "Failed to upload root metadata"
    rm -rf "${TEMP_DIR}"
    return 1
  fi
  ...
  execute_curl_with_retry "${ROOT_META_URL}.sha256" "PUT" "" "maven-metadata.xml.sha256"
  execute_curl_with_retry "${ROOT_META_URL}.sha512" "PUT" "" "maven-metadata.xml.sha512"
...
# Deprecated: Version-specific metadata updates are no longer used
# Commit history is now embedded in root maven-metadata.xml
# See update_root_metadata_with_commit_history() function
...
# Deprecated: JSON commit mapping is no longer used
# Commit history is now embedded directly in maven-metadata.xml
# See update_root_metadata_with_commit_history() function
...
publish_grammar_files() {
  ...
  # Update root metadata with commit history
