...
**Scope:** three benchmark pillars plus a type-coverage axis, run at fidelity scale
...
So we stood up a dedicated benchmark domain mirroring the customer's obs-pi pre-prod environment scaled down
...
| --- | --- | --- |
| **Perf** (§2.1) | What does each query cost on its own? | 96 templates × 6 time ranges = 576 measured points, single concurrency |
| **Load** (§2.2) | How does the cluster behave with N queries in flight? | concurrency ladder, stress ramp to N=100, 30-min sustained |
| **Use-case** (§2.3) | Do real users get a good experience? | 7 realistic workflows (dashboards, incident session, alerting, multi-user, WLM isolation) |
...
- **index granularity does not**: we use 10 fat indices spanning 7 days each; the customer has ~306 daily
  ...
  **cannot reproduce PIT-context exhaustion** (GH #5698/#5634), which needs >300 shards on one node.
...
**The plan's §4.2 Perf gates are not met.** Three categories miss them at fidelity scale: `rex`,
`simple-search`, `dedup`. Load (§4.3) and Use-case (§4.4) gates pass.
...
| ---: | --- | --- |
| **D1** | **Are `rex` / `dedup` / `simple-search` at ≥1 day release blockers, or documented known-slow with revised thresholds?** The §4.2 gates predate fidelity data. | Documented known-slow with revised gates, plus user-facing guidance. They are inherent costs, not regressions. |
...
| What | Where |
| --- | --- |
| Findings & action tracker | Chorus `ctWafvk3X0c2` |
| Plain-English walkthrough of the Use-case pillar | Chorus `sanzZe3QLdP3` · `results/fidelity-usecase-v1/EXPLAINED.md` |
| Perf report (576 points) | `results/fidelity-perf-v2/report.md` |
| Load report (ladder, ramp, sustained) | `results/fidelity-load-v1/report.md` + `-l3clean/` |
| Use-case reports | `results/fidelity-usecase-v1/`, `-v2-u5u7/` (corrected U5/U7), `-v3-wide/` (wildcard + 7 d) |
| Type × operation matrix (254 points) | `results/pushdown/report-types.md` |
| Reseed spec + expansion checklist | `RESEED-SPEC.md`, `TODO-EXPANSION.md` |
| Harness | `docs/dev/bench/perf-suite/`, branch `feature/ppl-perf-suite` |
